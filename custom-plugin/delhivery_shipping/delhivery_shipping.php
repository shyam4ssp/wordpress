...
function delhiveryShipping() {
	...
	if(isset($_POST['save_api_settings'])){
		...
		$wpdb->query($sql);*/
		$wpdb->update($wpdb->prefix.'delhivery_shipping_credentials', array('token'=>$api_token, 'mode'=>$api_mode, 'items_per_package'=>$items_per_package), array('id'=>1));
	...
	if(isset($_POST['save_pickup_settings'])){
		...
		$sql = $wpdb->insert($wpdb->prefix.'delhivery_pickup_location', array(
			'name' => $pickup_name,
			'phone' => $pickup_phone,
	...
	if(isset($_POST['save_shipment_returns_settings'])){
		...
		$sql = $wpdb->insert($wpdb->prefix.'delhivery_shipment_return', array(
			'name' => $shipment_name,
			'phone' => $shipment_phone,
			'pincode' => $shipment_pincode, 
