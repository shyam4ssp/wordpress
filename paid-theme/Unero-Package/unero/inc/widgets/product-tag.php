...
if ( ! class_exists( 'Unero_Widget_Product_Tag_Cloud' ) ) {

	/**
	 * Tag Cloud Widget.
	 *
	 */
	class Unero_Widget_Product_Tag_Cloud extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Your most used product tags in cloud format.', 'unero' );
			$this->widget_id          = 'unero_product_tag_cloud';
			$this->widget_name        = esc_html__( 'Unero Product Tags', 'unero' );
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Product Tags', 'unero' ),
		...
		public function _topic_count_text( $count ) {
			...
			return sprintf( _n( '%s product', '%s products', $count, 'unero' ), number_format_i18n( $count ) );
		}
